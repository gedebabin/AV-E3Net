...
def process_with_rtf(model: AVE3Net, vframes: torch.Tensor, noisy: torch.Tensor):
    print(vframes.shape, noisy.shape)
    # a = a.split(640, 1)[:-1]  # discard last incomplete chunk
    # v = F.interpolate(v.transpose(0, 3), (v.size(-1), len(a))).transpose(0, 3)
    # v = v.split(1)
    # v = [*v, v[-1]]

    # print(len(a), len(v), a[-1].shape, v.shape)
